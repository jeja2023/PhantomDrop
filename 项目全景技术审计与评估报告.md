...
> [!NOTE]
> **V0.0.36 可靠性增量（2026-07-20）**：Grok 注册新增工作流/系统/Cloudflare Zone 三级收信域名解析、受保护的就绪检查 API、管理端启动拦截、Chromium 动态参数发现回退和 Turnstile 级联容错；同时修复 Vite `/auth` 开发代理及 Windows `CRLF` 下发布工具漏改 Core lockfile 的问题。本增量不改变数据库结构和既有认证边界。
>
