...
> [!NOTE]
> **V0.0.39 代理兼容性增量（2026-07-21）**：Grok Chromium 参数发现与 Turnstile 回退新增 HTTP/HTTPS 代理认证拦截，代理凭据从浏览器命令行剥离并仅在 CDP 认证挑战中提交；同时明确认证 SOCKS 的 Chromium 能力边界。本增量不改变数据库结构或既有凭据存储边界。
>
> **V0.0.38 容器可靠性增量（2026-07-21）**：Grok 参数发现、Grok Turnstile 与 OpenAI 浏览器注册统一接入可配置的 Chromium sandbox；Docker 默认关闭进程 sandbox 以适配非特权容器，同时保持 UID 10001、无 `SYS_ADMIN` 的安全边界。浏览器协议依赖改为离线构建，镜像默认启动命令也不再依赖现场 Compose override。本增量不改变数据库结构。
>
> **V0.0.37 可靠性增量（2026-07-20）**：Grok 注册新增工作流/系统/Cloudflare Zone 三级收信域名解析、受保护的就绪检查 API、管理端启动拦截、Chromium 动态参数发现回退和 Turnstile 级联容错；同时修复 Vite `/auth` 开发代理及 Windows `CRLF` 下发布工具漏改 Core lockfile 的问题。本增量不改变数据库结构和既有认证边界。
>
> **V0.0.34 功能增量（2026-07-20）**：自动化引擎新增 Grok/xAI 协议注册、动态注册参数发现、字母数字 OTP 提取、Turnstile 多方案接入、受控批量并发和 SSO 资产落库；管理端、版本源与专项运行文档同步完成。该增量未改变 V0.0.33 建立的管理认证和 Worker 机器密钥边界，详细配置见 `docs/Grok自动注册说明.md`。
>
> **V0.0.33 审计整改闭环（2026-07-19）**：已完成管理认证与 `HUB_SECRET` 权限分离、Argon2id 密码哈希、Cookie 会话、登录限流、SSRF DNS 固定、敏感字段脱敏、事务型 Webhook Outbox、并发上限、工作流恢复、SSE 重同步、依赖与镜像锁定、CI 质量门禁及非 root 容器部署；并补齐账户设置职责分区、持久化开发数据库与旧生产库失败关闭升级保护。详细变更和升级要求见 `更新日志.md` 与 `VERIFY.md`。
