...
当前发布基线为 `V0.0.34`。在既有管理员认证、邮件中枢、持久化开发数据库与生产升级保护基础上，自动化引擎新增 Grok/xAI 协议注册：运行时发现注册参数、接收字母数字邮箱验证码、按配置处理 Turnstile、并发创建账号并将 SSO 写入账号库。管理端已提供对应配置入口，运行和取消继续复用统一工作流生命周期。
