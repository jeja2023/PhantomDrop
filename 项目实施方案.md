...
当前发布基线为 `V0.0.38`。Docker 中的 Grok/OpenAI Chromium 启动统一关闭进程 sandbox，解决 namespace 权限失败及后续调试端口误报；`headless_chrome` 同时切换为内建 CDP 的离线构建模式，并把 Xvfb 现场覆盖方式收回镜像默认启动命令。本版本不新增数据库迁移，也不扩大容器权限。
