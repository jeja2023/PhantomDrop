...
当前发布基线为 `V0.0.33`。管理端使用数据库持久化的管理员用户名与 Argon2id 密码哈希，`HUB_SECRET` 仅用于 Worker -> Hub `/ingest`；“账户与登录”与“网络与运行设置”已按职责拆分。本地开发使用 `core/phantom_core.db`，生产升级复用并备份现有命名卷，同时对旧认证密钥和历史重复工作流步骤执行失败关闭迁移保护。
