              ...
              <div className="grid flex-1 overflow-hidden lg:grid-cols-[1.1fr_1.5fr]">
                ...
                <div className="flex flex-col gap-6 overflow-y-auto p-8 custom-scrollbar">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <InfoCard 
                      label="归档状态" 
                      value={selectedEmail.is_archived ? '已归档' : '活跃中'} 
                      ...
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <div className="flex gap-2">
                          <button type="button" onClick={() => void toggleArchive(selectedEmail.id, !selectedEmail.is_archived)} className="phantom-btn phantom-btn--sm phantom-btn--secondary">
                            ...
                            {selectedEmail.is_archived ? '取消归档' : '归档'}
                          </button>
                          <button type="button" onClick={() => void deleteEmail(selectedEmail.id)} className="phantom-btn phantom-btn--sm phantom-btn--danger">
                            <Trash2 size={14} />
                            删除
                          </button>
                        </div>
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center justify-between rounded-xl bg-white p-4 border border-blue-100">
                          ...
                          {selectedEmail.extracted_code && (
                            <button
                              ...
                              className="h-12 w-12 flex items-center justify-center rounded-xl bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white transition-all shadow-sm"
                            >
                              <Copy size={20} />
                            </button>
                          )}
                        ...
                        <div className="rounded-xl bg-white p-4 border border-blue-100">
                          <div className="flex items-center justify-between mb-2">
                            <div className="text-[9px] font-bold text-blue-400 uppercase tracking-tighter">提取链接 / ACTION URL</div>
                          ...
                          {selectedEmail.extracted_link ? (
                            ...
                            <div className="text-sm font-medium text-slate-400 italic">未检测到有效跳转链接</div>
                          )}
                        </div>
                      </div>
                    </div>
                    <div className="absolute -right-4 -bottom-4 opacity-5 text-blue-600 pointer-events-none">
...
function InfoCard({
  ...
  return (
    <div className="group rounded-2xl border border-slate-200 bg-white p-4 transition-all hover:border-slate-300 hover:bg-slate-50/50">
      <div className="flex items-center justify-between gap-2 text-[10px] font-black tracking-widest text-slate-400 uppercase mb-1.5">
        <span>{label}</span>
        <div className="flex items-center gap-2">
      ...
      <div className={`break-all leading-snug ${emphasize ? 'font-mono text-lg font-black tracking-widest text-blue-600' : 'text-sm font-bold text-slate-900'} ${valueClassName}`}>
        {value}
      </div>
    </div>
  )
