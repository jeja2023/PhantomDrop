...
本文适用于 PhantomDrop `V0.0.39`，说明内建 `grok_register_default` 工作流的启用条件、配置边界和运行结果。注册协议参考 MIT 许可项目 [HSJ-BanFan/grok-register-web](https://github.com/HSJ-BanFan/grok-register-web)，并按 PhantomDrop 的邮件中枢、工作流和账号资产模型完成集成。
...
容器访问宿主机 Solver 时通常使用 `http://host.docker.internal:5072`。Chromium 回退支持 HTTP/HTTPS 用户名密码认证代理：命令行仅包含清理凭据后的代理地址，认证信息通过 CDP 挑战提交且不写入日志。Chromium 不支持 SOCKS 用户名密码认证，此类代理需要改用 HTTP/HTTPS 协议或通过本地无认证转发接入。
...
- **Chromium 无法启动**：检查容器/主机中的 Chrome、共享内存和执行权限。出现 `Failed to move to new namespace` 时确认 Docker 中 `PHANTOM_CHROME_SANDBOX=false`；认证 SOCKS 代理需改用 HTTP/HTTPS 代理或本地无认证转发。
